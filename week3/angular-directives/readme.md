...
## {{ }} - Angular Interpolation Markup (Mustaches)
...
```html
    <h1>
        <span class="number">{{ 2 + 2 }}</span> <!-- will be rendered as: 4 -->
        <span class="name">{{ students[0] + ' Smith' }}</span> <!-- will be rendered as: John Smith -->
    </h1>
```
...
These style values can also be dynamic. In other words, you can get the value of a variable (typically from a controller, but in this example, hardcoded as an initial value), and set your style properties to those values.
